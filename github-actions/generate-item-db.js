...
const locales = ['us', 'de', 'fr', 'ru', 'es', 'sp', 'pt', 'jp', 'kr', 'cn', 'tw', 'th', 'tr', 'id']
let totalDone = 0;
...
const scrape = async (locale) => {
    ...
    //let input = await fetch(`https://bdocodex.com/query.php?a=items&type=pearls&l=${locale}&_=${Date.now()}`).then(r => r.text());
...
const regions = ['na', 'eu', 'sea', 'console_eu', 'console_na', 'console_asia', 'mena', 'sa', 'kr', 'ru', 'jp', 'th', 'tw'];
const regionLinks = {
...
const scrapeCategories = async (region, category, subcategories) => {
    ...
    let result = await Promise.all(subcategories.map(c => {
        return fetch(marketUrl, {
            ...
            if (v.resultCode === 0)
                return { category, sub_category: c, data: v.resultMsg }

            else throw { message: 'maintenanceMaybe', statusCode: '_' + v.resultCode }
        })
    }))
